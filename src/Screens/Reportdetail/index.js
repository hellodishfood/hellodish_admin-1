  ...
  function Reportdetail() {
    ...
    async function fetchData() {
      ...
      const url = `${baseurl}customer/api/PaymentDetailsByRestaurantId/6685198a0f4c57b5a7c57752?startDate=${formattedStartDate}&endDate=${formattedEndDate}`;
      const response = await fetch(url, requestOptions);

      if (response.ok) {
        const responseData = await response.json();
        console.log("Fetched data:", responseData); // Log the fetched data to inspect it
