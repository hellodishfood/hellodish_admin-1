...
function Driver() {
  ...
  async function GetPDrivers() {
    setLoad(true);
    const result = await GetListData(PendingDriverApi);

    if (result.isOk && result.data?.data?.length > 0) {
      setLoad(false);
      console.log(result.data.data);
      setPdrivers(result.data?.data);
    } else {
      setLoad(false);
    }
  }
  ...
  async function GetDrivers() {
    const result = await GetListData(DriverApi);
    if (result.isOk && result.data?.data?.length > 0) {
      setDrivers(result.data?.data);
      setoDrivers(result.data?.data);
      setLoad(false);
      getCommision();
    } else {
      setLoad(false);
  ...
  async function Approve(driverid, status) {
    ...
    const result = await UpdateStatusApproveAndRejectStatus(
      `admin/api/updateDriverStatusApproveAndReject/${driverid}/${status}`
    );
    if (result.isOk) {
      GetDrivers();
      GetPDrivers();
  ...
  const search = (val) => {
    if (val !== "") {
      const searchResult = odrivers.filter(
        (user) =>
          user?.driver?.name?.toLowerCase().includes(val.toLowerCase()) ||
          user?.driver?.phone?.includes(val)
      );
      setaCurrentpage(1);
  ...
  const getCommision = () => {
    ...
    fetch(`${baseurl}admin/api/getDeliveryCharge`, requestOptions)
      ...
      .then((result) => {
        ...
        if (result.status === true) {
          ...
          setIsmulti(result.data.multipleOrder);
  ...
  const UpdateComission = () => {
    ...
    fetch(`${baseurl}admin/api/updateDeliveryCharge`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result.status === true) {
          getCommision();
            ...
            <div className="row">
              <div className="col-12">
                <div className="row mb-4">
                  <div className="col-lg-4 col-md-6 drivers-btn-top">
                    <div className="d-flex align-items-center gap-3">
                      <h4 className="font-w600">Multiple</h4>
                      <h4>Order Accept for All Drivers</h4>
                    </div>
                    <div className="d-flex drivers-btn align-items-center gap-3">
                      <button
                        type="button"
                      ...
                      <div className="form-check form-switch">
                        <input
                          ...
                          checked={ismulti === 1}
                          onChange={() => {
                            ismulti === 0 ? setIsmulti(1) : setIsmulti(0);
                          }}
                        />
                      </div>
                    </div>
                  </div>
                  <div className="col-lg-8  col-md-6 drivers-btn-top">
                    <div className="d-flex align-items-center gap-3 gap-md-2">
                      <h4 className="font-w600">Commision</h4>
                      <h4>Driver</h4>
                    </div>
                    <div className="d-flex align-items-center drivers-btn">
                      <div className="d-flex align-items-center gap-3 gap-md-2 w-lg-50">
                        <button
                          type="button"
                          className="btn btn-success1 pending my-1"
                        >
                          Per Order
                        </button>
                        <div className="form-check form-switch">
                          <input
                            ...
                            checked={type === 1}
                            onChange={() => {
                              type === 1 ? setType(0) : setType(1);
                            }}
                          />
                        </div>
                      </div>
                      <div className="d-flex align-items-center gap-3 w-lg-50">
                        <button
                          type="button"
                          className="btn btn-success1 pending my-1"
                        >
              ...
              <div className="col-12">
                <div className="d-flex align-items-center gap-3">
                  <h4 className="font-w600">{pdrivers?.length}</h4>
                  <h4>New Driver pending request</h4>
                </div>
              </div>
                          ...
                          <div className="col-3">
                            <small className="text-black">
                              {item.driver.profileImage !== "" ? (
                                <img
                                  src={`${baseurl}${item.driver.profileImage}`}
                                  alt=""
                                />
                              ) : (
                ...
                <div className="row menu-page">
                  <div className="col-12 mb-4">
                    <div className="d-flex align-items-center gap-2 approved-drivers">
                      <p className="mb-0">
                        <b className="text-black">{drivers.length} Drivers</b>
                      </p>
